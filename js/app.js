(()=>{
  ...
  const cameraBtn=document.getElementById('cameraBtn'), scanBtn=document.getElementById('scanBtn'),
        ...
        rawInput=document.getElementById('rawQrInput'), parseRawBtn=document.getElementById('parseRawBtn'),
  ...
  function resetAll(){ if(state.stream){ state.stream.getTracks().forEach(t=>t.stop()); state.stream=null; }
    videoEl.classList.remove('hidden'); dynFields.innerHTML=''; compiledPrompt.value=''; infosEl.textContent='';
    iaButtons.innerHTML=''; ficheMeta.textContent='Catégorie – Titre – Version – QR CODE flashé';
    state={ qr:null, stream:null, capturedPhotos:{}, gps:null };
  }
  const parseJSONSafe=(s)=>{ try{return JSON.parse(s);}catch(e){ toast('JSON invalide.'); return null; } };
  function updateFicheMeta(){ const q=state.qr; if(!q) return; ficheMeta.textContent=`${q.categorie||'–'} – ${q.titre_fiche||'–'} – ${q.version||'–'} – QR OK`; }
  function updateInfosComplementaires(){ const q=state.qr; if(!q) return;
    const refs=Array.isArray(q.references_bibliographiques)? q.references_bibliographiques.join(', '):'';
    const suivi=q.infos_complementaires||''; const objectif=q.objectif?`Objectif: ${q.objectif}\n`:''; const refsTxt=refs?`Références: ${refs}\n`:'';
    infosEl.textContent=`${objectif}${refsTxt}${suivi}`.trim(); }
  function createField(f){ const wrap=document.createElement('div'); wrap.className='field';
    ...
    switch(f.type){
      ...
      case 'textarea': input=document.createElement('textarea'); input.id=f.id; input.rows=f.rows||3; wrap.appendChild(input); break;
      case 'radio':
      ...
      case 'select':
        ...
        (f.options||[]).forEach(opt=>{ const o=document.createElement('option'); o.value=opt.value; o.textContent=opt.label; input.appendChild(o); });
        wrap.appendChild(input); break;
      case 'gps': input=document.createElement('div'); input.id=f.id; input.textContent=t('gpsNotAcquired'); wrap.appendChild(input); break;
      case 'photo':
      ...
      default: input=document.createElement('input'); input.type='text'; input.id=f.id; wrap.appendChild(input);
  ...
  function getFieldValue(f){ if(f.type==='gps') return state.gps? `${state.gps.lat}, ${state.gps.lon} (±${Math.round(state.gps.accuracy)} m)` : '';
  ...
  function compilePrompt(){ if(!state.qr) return '';
    const values={}; (state.qr.champs_entree||[]).forEach(f=> values[f.id]=getFieldValue(f));
    let template=state.qr.prompt||''; Object.entries(values).forEach(([k,v])=>{ const re=new RegExp(`{{\\s*${k}\\s*}}`,'gi'); template=template.replace(re, v||''); });
    ...
    const full=[header, template, '', '# Données saisies', bloc].join('\n').trim(); compiledPrompt.value=full; return full; }
  function openIA(key, meta){ const prompt=compilePrompt();
    const url=(meta.url && meta.url.replace('%q%', encodeURIComponent(prompt))) || `https://chat.openai.com/?q=${encodeURIComponent(prompt)}`;
    createBundle(true); window.open(url, '_blank'); if(meta.client_uri){ setTimeout(()=> window.location.href=meta.client_uri.replace('%q%', encodeURIComponent(prompt)), 300); } }
  function renderIABtns(){ iaButtons.innerHTML=''; const q=state.qr; if(!q||!q.ia_cotation) return;
    Object.entries(q.ia_cotation).forEach(([key,meta])=>{ const score=Number(meta.score||0); if(score<=1) return;
      const b=document.createElement('button'); b.className='ia-btn '+(score===3?'green':'orange'); b.textContent=meta.label+(meta.paid?(' '+t('paidVersion')):''); b.addEventListener('click', ()=>openIA(key, meta)); iaButtons.appendChild(b); }); }
  geoBtn.addEventListener('click', ()=>{ if(!('geolocation' in navigator)) return toast('Géolocalisation non disponible.');
  ...
  qrFile.addEventListener('change', async e=>{ const file=e.target.files?.[0]; if(!file) return; if(!('BarcodeDetector' in window)) return toast(t('noBarcode'));
    const bmp=await createImageBitmap(file); if(!detector) detector=new window.BarcodeDetector({formats:['qr_code']}); const codes=await detector.detect(bmp);
  ...
  function handleQrRaw(raw){ let jsonStr=raw; try{ if(raw.startsWith('data:application/json')){ const b64=raw.split(',')[1]; jsonStr=atob(b64); } }catch(_){}
    const obj=parseJSONSafe(jsonStr); if(!obj) return; state.qr=obj; updateFicheMeta(); updateInfosComplementaires(); renderFields(); compilePrompt(); renderIABtns();
    if(state.stream){ state.stream.getTracks().forEach(t=>t.stop()); state.stream=null; } videoEl.classList.add('hidden'); }
  document.getElementById('copyPromptBtn').addEventListener('click', ()=>{ compilePrompt(); navigator.clipboard.writeText(compiledPrompt.value).then(()=>toast(t('promptCopied'))); });
  function createBundle(silent=false){ const parts=['--- prompt.txt ---\n', compiledPrompt.value+'\n\n'];
